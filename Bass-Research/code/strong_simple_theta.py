...
class StrongSimpleTheta:
    ...
    def fit(self, y: np.ndarray) -> 'StrongSimpleTheta':
        ...
        # Step 4: Estimate drift from seasonally adjusted series
        self.drift = _estimate_drift(y_sa, window=min(52, n))
    ...
    def forecast_point(self, horizon: int) -> ThetaResult:
        ...
        # Fit linear trend to seasonally adjusted series
        x = np.arange(n)
        ...
        # This emphasizes the most recent level
        theta2_forecast = self.ses_level + self.drift * horizon
